...
print 'Loading DataClass dependencies...'
...
#import locally the Range Function
from Utility_Main           import Range
...
#Non negative routine fact management
import Utility_PCA          as PCA

#Non negative routine fact management
import Utility_NMF_R        as NMF_R

#------------------------------------------------#

#Data information class upon load
import Data_DataInfo        as DataInfo

#------------------------------------------------#

#the fitting class will be intialised
import Fitting.Raman_Fit    as RamFit

#------------------------------------------------#

#the fitting class will be intialised
from IO_Write               import Write

#the fitting class will be intialised
#from IO_Read                import Read
...
class Data:
    ...
    def __init__(self):
        ...
        #initialize the Write and read classes
        self.Write      = Write(self)
        #self.Read       = Read()
    
    def Add_Fit_Class(self, Python_Class):
        """
        #######################################################################
        ...
        self.Main_Fit_Class = Python_Class
    
    
    def Load_RamFit(self):
        """
        #######################################################################
        Loads the fiting class and clears it in case it is already defined
        #######################################################################
    ...
    def LoadRaw(self,Path,PathOut,Input,Container,event = None,queue = None):
        """
        #######################################################################
        ...
        array, gather the information from the user and finnally dump it. Note
        ...
        while not BreakFree:
            ...
            if int(Input[0]) == 5:
                ...
                omega,Data,Act = Utility.ReadSingleFile(Path)
            
                #Build header
                HeadStr,HeadStrEx = self.Write.PrepWrite(Path,'Single')
                ...
                #Build the Activeset
                Activeset = [False,False,False,False]
            ...
            #We have a Temperature series
                ...
                for idx,Val in enumerate(PathList):
                    ...
                    #Actually append the value
                    T.append(float(Val.split(os.path.sep)[-1].split(self.temp[0])[1].split(self.temp[1])[0]))
                ...
                #Depth measurement we only need to order according to Z
                T, PathListFix = zip(*sorted(zip(T, PathList)))
                ...
                #Build the ouptu
                LastAct += "\nData was successfully loaded and rearanged:"
                LastAct += "\n- Dataset Range: "+str(numpy.min(T))+" to "+str(numpy.max(T))+" micrometer"
                if int(Input[0]) == 0:
                    LastAct += "\n- Dataset Steping: "+str((numpy.max(T)-numpy.min(T))/len(T))+" micrometer"
                LastAct += "\n- Number of files: "+str(len(T))
                    
                    
                #Process Signal (version 0.0.4 omited the filtering options, can be readded maybe later... )
                Data, omega = self.SignalProcV2(Data,
                                                Omega,
                                                Z,
                                                X,
                                                Y,
    ...
    def ImportFileDataV2(self,DataPath,Z,X,Y,T,event,queue):
        ...
        for File in DataPath:
            ...
            for Line in f:
                ...
                #create omega
                #french stupids use comma remove them
                
                try:
                    
                    OmegaTemp.append(float(Line.strip().replace('\t',' ', 1).replace(',','.').split()[0]))
                
                except:
                    
                    pass
        
            #append the data to the Omega lise
            Omega.append(list(OmegaTemp))
                ...
                for k in range(0,len(Y)):
                    ...
                    for l in range(0,len(T)):
                        ...
                        for Line in f:
                            ...
                            try:
                                transfer.append(float(Line.strip().replace('\t',' ', 1).replace(',','.').split()[1]))
                            except:
                                pass
                        #save the data
                        Data.append(list(transfer))
        ...
        '''
        ##############################################################################
        Final check. Note that if omega is inverted we need to fix itself and the 
        included data.
        ##############################################################################
        '''
    ...
    def FixOmegaData(self,omega, Min, Max):
        ...
        for Element in omega:
            ...
            for g in range(1,len(Element)):
                
                #get difference
                Delta.append(Element[g]-Element[g-1])   #We take the position
        
            
        #take the average
        CorStep = numpy.abs(numpy.mean(Delta))
        ...
        while NotReachMin:
            ...
            #check
            if i * CorStep + Min > Max:
                
                NotReachMin = False
                
                break
            
            #Add an element to our new X Data array
            CorXPoints.append(i * CorStep + Min)
    ...
    def SignalProcV2(self,Data,omega,Z,X,Y,T,Activeset,event,queue):
        ...
        #find the dataset we wil extract at the end of the manipulations
        Middle = int(self.mult/2)
        
        
        '''
        ##############################################################################
        Preprocessing Sometimes dataset do not have the same wavenumber value. This
        makes it rather difficult to process them. Here a correction routine will run
        over the values and grab the indexes
        ##############################################################################
        '''
        ...
        for Element in omega:
            ...
            if numpy.min(Element) > Min:
        
                Min = numpy.min(Element)
        
            if numpy.max(Element) < Max:
        
                Max = numpy.max(Element)
    
        
        #Find Index range in the provided 2D data array
        #omega, Data = self.FindIdxV2(omega,Data)
        omegaTemp = self.FixOmegaData(omega, Min, Max)
            ...
            for j in range(0,len(X)):
                ...
                for k in range(0,len(Y)):
                    ...
                    for l in range(0,len(T)):
                        ...
                        #load the current omega
                        CurrentOmega    = omega[Index]
                        CorrectedData   = numpy.zeros((len(omegaTemp)))
                        
                        for m in range(len(omegaTemp)):
                            ...
                            #grab the index
                            n_1, n_2 = Utility.FindIdx3(omega0, CurrentOmega)

                            print n_1, n_2, omega0

                            #Check if the value is in between two consecutive points
                            if CurrentOmega[n_1] == omega0:
                                    
                                CorrectedData[m] = ToCorrectData[n_1]
                                
                            else:

                                #Get the spacing between our two points:
                                XSpacing = CurrentOmega[n_2]-CurrentOmega[n_1]
                                ZSpacing = ToCorrectData[n_2]-ToCorrectData[n_1]
                        
                                #Get the actual offset
                                XOffsetSpacing = omega0-CurrentOmega[n_1]
                                
                                
                                #buffer it
                                CorrectedData[m] = ZSpacing/XSpacing*XOffsetSpacing+ToCorrectData[n_1]
                        ...
                        #Otherwise just copy and move on
                            ...
                            PreSigProcOutput.append(CorrectedData)
    ...
    def Export(self,Data,omega,Z,X,Y,T,Activeset,Type,Path,PathOut,event,queue):
        """
        ###########################################################################################        
        ...
        self.HeadStr,self.HeadStrEx = self.Write.PrepWrite(Path,Type)

        #Set tail
        Tail = self.HeadStr

        #Send it out
        self.Write.Write2FileV2(self,Data,Z,X,Y,T,Type,Activeset,omega,Tail,PathOut = PathOut,event = event,queue = queue)
    ...
    def Load(self,Path):
        ...
        while not Broken:
            ...
            try:
                ...
                print 'Could not import the file'
                break
            ...
            self.MeasInfo   = '**Measurement_Information**'
            ...
            for i in range(0,len(Read)):
                ...
                #find raman data
                if SplitLine[0] == self.RamInfo:
                    FoundIdx[0] = i
                    LastAct += '\nFound the Raman Information section in the selected file:\nLine number '+str(i)+', proceeding...'
                        
                #find the sample information
                if SplitLine[0] == self.SamInfo :
                    FoundIdx[1] = i
                    FoundIdx[20] = i #just in case there is no measurement informtation
                    LastAct += '\nFound the Sample Information section in the selected file:\nLine number '+str(i)+', proceeding...'
                
                #find the sample information
                if SplitLine[0] == self.MeasInfo:
                    FoundIdx[20] = i
                    LastAct += '\nFound the Measurment Information section in the selected file:\nLine number '+str(i)+', proceeding...'
            ...
            for line in Read:
                ...
                if u == 0:
                    ...
                    if self.Type == 'Component' or self.Type == 'Score':
                ...
                ######################################################################
                #Raman Information
                elif u >= FoundIdx[0]+1 and u < FoundIdx[1]:
                    
                    RamanData += '**'+line
                    self.RamInfo    += '\n'+line
            
                #Sample information
                elif u >= FoundIdx[1]+1 and u < FoundIdx[20]:
                    
                    SampleData += '**'+line
                    self.SamInfo    += '\n'+line
                
                #Sample information
                elif u >= FoundIdx[20]+1 and u < FoundIdx[20]+4:
                    
                    MeasurementData += '**'+line
                    self.MeasInfo    += '\n'+line
                    ...
                    try:
                        ...
                        print 'ERROR in processing the import'
                        pass
        ...
        try:
            ...
            LastAct += '\old version no measurement info'

        try:
        
            LastAct += self.SetMeasurement(MeasurementData)
        
        except:
        
            LastAct += '\n Old version no measurement information'
    ...
    def SetMeasurement(self,Input):
        ...
        #Initialise arrays
        self.MeasurementID    = [0]*InputLines
        self.MeasurementVal   = [0]*InputLines
        ...
        for i in range(1,len(InputSplit)):
            ...
            if len(TempInput)<2:
                ...
                #Insert values
                self.MeasurementID[i]   = TempInput[0]
                self.MeasurementVal[i]  = TempInput[1]
        
        #Load into local class variables
        self.MeasurementName        = self.MeasurementVal[Utility.RetIdx('Name'      ,self.MeasurementID)[0]]
        self.MeasurementDate        = self.MeasurementVal[Utility.RetIdx('Date_0'    ,self.MeasurementID)[0]]
        self.MeasurementProcDate    = self.MeasurementVal[Utility.RetIdx('Date_1'    ,self.MeasurementID)[0]]
    ...
    def SampleInfoInit(self):
        ...
        try:
            if self.Type == 'Depth':
                ...
                self.Info.SampleInfoVal.append(str(self.T.Num))
        ...
        self.Info.SampleInfoVal.append(str(self.RamGrating))
        ...
        try:
            if not self.Y.Y[0] == [None]:
                ...
                self.Info.SampleInfoUnit.append('')
        except:
            self.Info.SampleInfoVal.append('Fixed')
            self.Info.SampleInfoUnit.append('')
        ...
        try:
            if not self.T.T == [None]:
                ...
                #We can set the values
                self.Info.SampleInfoVal.append(str(self.T.TMin)+' to '+str(self.T.TMax))
        ...
        ########################
        ########################
        #Measurement info trial
        
        try:
            
            #time per acquisition
            self.Info.SampleInfoID.append('Name')
            self.Info.SampleInfoVal.append(str(self.MeasurementName).replace('_',' '))
            self.Info.SampleInfoUnit.append(None)
            
            #time per acquisition
            self.Info.SampleInfoID.append('Acquisisiton date')
            self.Info.SampleInfoVal.append(str(self.MeasurementDate))
            self.Info.SampleInfoUnit.append(None)
            
            #time per acquisition
            self.Info.SampleInfoID.append('Processing date')
            self.Info.SampleInfoVal.append(str(self.MeasurementProcDate))
            self.Info.SampleInfoUnit.append(None)
            ...
            #time per acquisition
            self.Info.SampleInfoID.append('Name')
            self.Info.SampleInfoVal.append('No_Name')
            self.Info.SampleInfoUnit.append(None)
            
            #time per acquisition
            self.Info.SampleInfoID.append('Acquisisiton date')
            self.Info.SampleInfoVal.append('')
            self.Info.SampleInfoUnit.append(None)
...
class XAxis:
    ...
    This has been changed in version 0.2.02 as sometimes data patching is
    recquired. Note that this still should allow for backward compatibility.
    Now the structure will be an array of arrays. That are inerarly related
    to the datastructure arrangement.
    #######################################################################
    """
...
class YAxis:
    ...
    def getMultIdx(self,ValArray):
        ...
        try:
            ...
            for i in range(0,3):
                try:
                    idx[i] = int(Utility.FindIdx2(float("{0:.9f}".format(ValArray[i])),self.Y[i]))
                except:
                    pass
...
class PCAClass:
    ...
    def __init__(self,DataClass):
        ...
        self.Calc = PCA.PCA(copy = True, whiten = False)
    ...
    def Export(self):
        ...
        #Prep the write header
        self.HeadStr,self.HeadStrEx = self.Write.PrepWrite(self.DataClass.Path,self.Type)

        #Set tail
        Tail = 'NoiseProc'

        #Send it out
        self.Write.Write2FileV2(self.DataClass,
                           Data,
                           Z,X,Y,T,
                           self.Activeset,
                           omega,
                           Tail,
                           PathOut = self.DataClass.DirPath)
...
class ContourClass:
    ...
    def __init__(self,DataClass):
        ...
        #Define the initial upper and lower (first representation always spectral)
        self.Lower = [DataClass.X.XMin,
                      DataClass.Y.YMin[0],
                      DataClass.Y.YMin[1],
                      DataClass.Y.YMin[2],
                      DataClass.T.TMin]
                      
        self.Upper = [DataClass.X.XMax,
                      DataClass.Y.YMax[0],
                      DataClass.Y.YMax[1],
                      DataClass.Y.YMax[2],
                      DataClass.T.TMax]
    ...
    def BuildProjection(self,Option = 'Omega'):
        ...
        Val[4,:] = [self.Lower[4],self.Upper[4]]
        ...
        #Get the index values from space and from X
        self.Idx[4,0],Buffer = self.DataClass.T.getIdx(self.Lower[4])
        
        #log it
        LastAct += Buffer
        
        self.Idx[4,1],Buffer = self.DataClass.T.getIdx(self.Upper[4])
        ...
        if Option == 'Omega':
            ...
            X = Omega[int(numpy.min(self.Idx[0,:])):int(numpy.max(self.Idx[0,:]))]
            ...
            if self.Type == 'Depth':
                ...
                Max = len(self.DataClass.Y.Y[0])
                    
            elif self.Type == 'Temperature':
            
                Max = len(self.DataClass.T.T)
            ...
            ########################################################################
            ########################################################################
            #Here we have to do a type check. If the type is temperature measurement
            #we can drop the complicated distance calculations...
            
            if self.Type == 'Depth':
                
                #Axis
                YIdxList = [None]*int(Max)
                YValList = [None]*int(Max)
                
                
                #Compute it
                for i in Range(0,int(Max)):
                    '''
                    YValList[i] = [Val[1,0]+i*StepZ,
                                   Val[2,0]+i*StepX,
                                   Val[3,0]+i*StepY]
                    ...
                    YValList[i] = [self.DataClass.Y.Y[0][i],0,0]
                    YIdxList[i] = [i,0,0]
                ...
                for i in Range(0,int(Max)):
                    ...
                    Y.append(numpy.sqrt((YValList_Array[i,0]-YValList_Array[0,0])**2+
                                        
                                        (YValList_Array[i,1]-YValList_Array[0,1])**2+
                                        
                                        (YValList_Array[i,2]-YValList_Array[0,2])**2))
                ...
                #Axis
                TIdxList = [None]*int(Max)
                TValList = [None]*int(Max)
                
                #Compute it
                for i in Range(0,int(Max)):
                    
                        
                    TValList[i] = self.DataClass.T.T[i]
                
                    TIdxList[i] = self.DataClass.T.getIdx(TValList[i])[0]
                
                #remove None from array
                TValList_Array = numpy.nan_to_num(TValList)
                TIdxList_Array = numpy.asarray(TIdxList)
            ...
            #initiate Z
            Z = []
            RangeFetch = [int(numpy.min(self.Idx[0,:])),int(numpy.max(self.Idx[0,:]))]
            ...
            if self.Type == 'Depth':
                ...
                for i in Range(0,int(Max)):
                    ...
                    Z.append(self.DataClass.Z.getSpectraAtIdx(YIdxList_Array[i],
                                                              0,
                                                              Range = RangeFetch))
                ...
                for i in Range(0,int(Max)):
                    ...
                    Z.append(self.DataClass.Z.getSpectraAtIdx([0,0,0,TIdxList_Array[i]],
                                                               0,
                                                               Range = RangeFetch))
    ...
    def InitiateContour(self):
        ...
        self.CropVali = [self.X[self.CropIdxi[0]],
                         self.X[self.CropIdxi[1]],
                         self.Y[self.CropIdxi[2]],
                         self.Y[self.CropIdxi[3]]]
