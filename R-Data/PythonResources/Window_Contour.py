...
print 'Loading Contour dependencies...'
...
'''
###########################################################################
Import All Tkinter python dependecies
###########################################################################
'''
...
class DrawContour:
    ...
    def __init__(self,Data,root):
        ...
        #Check if the Contour class exists already in Data
        if not Data.isContour:
        
            #if not load the contour
            Data.LoadContour()
        ...
        self.ContourWindow = tk.Toplevel(self.master)
        
        #binf the methofd to a destroy
        self.ContourWindow.protocol("WM_DELETE_WINDOW", self.on_closing)
        
        #lanuch the window class dependency
        app = ContourWindow(self.ContourWindow ,Data)

    def on_closing(self):
        
        print 'Destroy...'
        
        self.ContourWindow.destroy()

        del self.ContourWindow
...
class ContourWindow():
    ...
    def __init__(self, root, DataClass):
        ...
        self.master.title("Contour Interface ("+self.DataClass.Info.GetInfoVal('Name')+")")
    ...
    def populate(self,root):
        ...
        self.PCABut[1] = ttk.Checkbutton(self.MidLabelFrame, text = 'Contour'  )
        ...
        self.BuilderClass[0] = GridWindowClass(self.ToggleFrame[0],self.DataClass,self,'Contour', isWindow = False )
        ...
        self.BuilderClass[3] = RangeWindow(    self.ToggleFrame[3],self.DataClass,self,'Contour', isWindow = False )
        ...
        self.CommandList = [partial(self.Contour,0),
                            partial(self.Contour,1),
                            partial(self.Contour,2),
                            partial(self.Contour,3)]
    ...
    def CreateFigureSimplePlot(self):
        ...
        ################################
        #Define the subpllots
        self.ax = mycanvas.GetSubPlot(1,0)
        self.bx = mycanvas.GetSubPlot(0,0)
        self.cx = mycanvas.GetSubPlot(1,1)
        self.dx = mycanvas.GetSubPlot(0,1)
        ...
        ################################
        #initialise the contour plot
        self.ax.Axes.isYSci          = [True,True]
        self.ax.Axes.isXSci          = [True,True]
        ...
        self.ax.Axes.TicksActiveGrid  = [True, True, True, True]
        self.ax.Axes.LabelsActiveGrid = [True, True, False, False]
        self.ax.Axes.PaddingOut       = [0.1, 0.05, 0.002, 0.002]
        
        self.ax.Pointer.Sticky = 4
        self.ax.Axes.Thickness = Thickness
        
        ################################
        #initialise the top side plot
        self.bx.Axes.isYSci          = [True,True]
        self.bx.Axes.isXSci          = [True,True]
        
        self.bx.Axes.XTickSpacing         = 50
        self.bx.Axes.XTickType            = 1
        
        self.bx.Pointer.YSciPrecision = '%.2e'
        self.bx.Pointer.XSciPrecision = '%.2e'
        
        self.bx.Axes.TicksActiveGrid  = [True, False, False, True]
        self.bx.Axes.LabelsActiveGrid = [True, False, False, True]
        self.bx.Axes.PaddingOut       = [0.1, 0.0, 0.002, 0.2]
        
        self.bx.Pointer.Sticky = 1
        self.bx.Axes.Thickness = Thickness
        
        ################################
        #initialise the right side plot
        self.cx.Axes.isYSci          = [True,True]
        self.cx.Axes.isXSci          = [True,True]
        
        self.cx.Axes.YTickSpacing         = 0.2
        self.cx.Axes.XTickNumber          = 4
        self.cx.Axes.YTickType            = 1
        
        self.cx.Pointer.YSciPrecision = '%.2e'
        self.cx.Pointer.XSciPrecision = '%.2e'
        
        self.cx.Axes.TicksActiveGrid  = [False, True, True, False]
        self.cx.Axes.LabelsActiveGrid = [False, True, True, False]
        self.cx.Axes.PaddingOut       = [0.0, 0.05, 0.3, 0.002]
        
        self.cx.Pointer.Sticky = 3
        self.cx.Axes.Thickness = Thickness
        ...
        self.bx.BindCursor()
        self.bx.Axes.PlaceGrids()
        self.bx.BindZoomer()
        
        self.cx.DrawAllPlot()
        self.cx.Axes.DrawAxes()
        self.cx.Axes.PlaceAllLabels()
        self.cx.BindCursor()
        self.cx.Axes.PlaceGrids()
        self.cx.BindZoomer()
        ...
        ################################
        #link up the axes between the contour and suplot
        mycanvas.Link(self.ax,self.bx,variableIn = 'x',variableOut = 'x')
        mycanvas.Link(self.ax,self.cx,variableIn = 'y',variableOut = 'y')
        mycanvas.Link(self.bx,self.ax,variableIn = 'x',variableOut = 'x')
        mycanvas.Link(self.cx,self.ax,variableIn = 'y',variableOut = 'y')
    ...
    def Spectra(self):
        ...
        self.base = BaseWindowClass(self.BaseWindow,self.DataClass,self,'Contour')
    ...
    def onclick(self,event):
        ...
        #start the veent handler and launch the view
        
        if event.xdata != None and event.ydata != None:
            
            self.RunSides(event.xdata, event.ydata)
            
            self.PCALabels[1].config(text = 'x = '+str(round(self.LineRange[0],2)))
            self.PCALabels[2].config(text = 'y = '+str(round(self.LineRange[1],2)))
    ...
    def RunSides(self,x,y):
        ...
        #Find Ids
        self.PickIdx[1]  = Utility.FindIdx(self.DataClass.Contour.YPlot,float(self.LineRange[1]),"Y")
        self.PickIdx[0]  = Utility.FindIdx(self.DataClass.Contour.XPlot,float(self.LineRange[0]),"X")

        #fetch plots
        self.YSidePlotX = self.DataClass.Contour.YPlot[self.PickIdx[0],:]
        self.ZSidePlotX = self.DataClass.Contour.ZPlot[self.PickIdx[0],:]
        self.XSidePlotY = self.DataClass.Contour.XPlot[:,self.PickIdx[1]]
        self.ZSidePlotY = self.DataClass.Contour.ZPlot[:,self.PickIdx[1]]
    
        self.canvas.takefocus()
    ...
    def Refreshax3D(self):
        '''
        ######################################################################
        Refresh the main contour
        ######################################################################
        '''
        #clear current draw
        pass
    ...
    def Refreshax(self):
        ...
        if self.CPerform and self.Perform:
        
            #build the contour plot in ax
            self.Contour = self.ax.AddContour(#The Data input
                                              self.DataClass.Contour.XPlot.tolist(),
                                              self.DataClass.Contour.YPlot.tolist(),
                                              self.DataClass.Contour.ZPlot.tolist(),
                                              ...
                                              #for the mesh
                                              MeshStepping = self.CNumber,
                                              MeshThickness = self.CLineWidth,
                                              MeshColorList = [self.CColorStr,
                                                               self.CColorStr ])
        
        elif not self.CPerform and self.Perform:
        ...
        elif self.CPerform and not self.Perform:
        ...
        #print '##################\nSizes :'
        #print 'X -> ',len(self.DataClass.Contour.XPlot.tolist()), len(self.DataClass.Contour.XPlot.tolist()[0])
        #print 'Y -> ',len(self.DataClass.Contour.YPlot.tolist()), len(self.DataClass.Contour.YPlot.tolist()[0])
        #print 'Z -> ',len(self.DataClass.Contour.ZPlot.tolist()), len(self.DataClass.Contour.ZPlot.tolist()[0])
        '''
        ...
        self.bx_projectionPlot = self.bx.AddProjectionPlot(self.Contour,
                                                           Type = 'x')
            
        self.cx_projectionPlot = self.cx.AddProjectionPlot(self.Contour,
                                                           Type = 'y')
        
        #link it all
        self.ax.Pointer.Projections = [[self.bx_projectionPlot,
                                       self.bx],
                                       [self.cx_projectionPlot,
                                       self.cx]]
    def RefreshaxT(self):
        '''
        ######################################################################
        This refresher will manage the default overlay for the contourplot
        this allows to not refresh all the time the main contour and
        still edit some visual characteristics
        ######################################################################
        '''
        #clear current draw
        self.axT.cla()
        
        #Check if we have to put lines
        if self.PutLines:
            
            VisOut.TextBox(Title = 'Action', Text = 'The user selected: '+str( self.LineRange[0])+' '+str(self.LineRange[1]), state = 1)
            self.axT.axhline(self.LineRange[1], color = self.LineRange[5],linewidth = float(self.LineRange[6]))
            self.axT.axvline(self.LineRange[0], color = self.LineRange[5],linewidth = float(self.LineRange[6]))
    ...
    def Refreshbx(self):
        ...
        #Add the line in continuation with the contour plot
        self.bxArtist , = self.bx.plot(self.MainCursorHandler.XSidePlotY,self.MainCursorHandler.ZSidePlotY,clip_on = True, linewidth = 2.5, color = 'black')
    
        #Introduce limits according to the maxima (can be enhanced)
        self.bx.set_xlim(numpy.min(self.MainCursorHandler.XtSidePlotY),numpy.max(self.MainCursorHandler.XSidePlotY))
    
        #Set ticks for the top plot                
        self.bx.set_xticks(self.DataClass.Contour.XTicks)
    
        #Handle ticks and labels and grid
        self.bx.xaxis.grid(True,'major',linewidth=1)
        
        labels = self.bx.get_xticklabels()
        for label in labels:
            label.set_rotation(270)

        self.bx.xaxis.set_ticks_position('top')
    ...
    def Refreshcx(self):
        ...
        '''
        #Add the line in continuation with the contour plot
        self.cxArtist, = self.cx.plot(self.MainCursorHandler.ZSidePlotX,self.MainCursorHandler.YSidePlotX,clip_on = True, linewidth = 2.5, color = 'black')
    
        #Introduce limits according to the maxima (can be enhanced)
        self.cx.set_ylim(numpy.min(self.MainCursorHandler.YSidePlotX),numpy.max(self.MainCursorHandler.YSidePlotX))
    
        #Set ticks for the top plot                
        self.cx.set_yticks(self.DataClass.Contour.YTicks)
    
        #Handle ticks and labels and grid
        self.cx.yaxis.grid(True,'major',linewidth=1)
        
        labels = self.cx.get_xticklabels()
        for label in labels:
            label.set_rotation(270)

        self.cx.yaxis.set_ticks_position('right')
    ...
    def Contour(self,Target):
        
        '''
        ######################################################################
        Seting the contours for furthe utilisation
        ######################################################################
        '''
    ...
    def Range(self):
        ...
        #lanuch the window class dependency
        self.app3 = RangeWindow(self.RangeWindow,self.DataClass,self,'Contour')
